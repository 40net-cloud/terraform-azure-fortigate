...
#       {
#         fgt_index      = i
#         name          = "${var.prefix***REMOVED***-fgt-${i+1***REMOVED***-MGMT-HTTPS"
#         protocol      = "Tcp"
...
#       {
#         fgt_index      = i
#         name          = "${var.prefix***REMOVED***-fgt-${i+1***REMOVED***-MGMT-SSH"
#         protocol      = "Tcp"
...
resource "azurerm_network_interface_nat_rule_association" "nat_assoc" {
  for_each = merge({
    ...
    format("%s-fgt-%d-MGMT-SSH", var.prefix, idx + 1) => {
      network_interface_id = module.fgt.fortigate_network_interface_external["node-${idx***REMOVED***"].id
