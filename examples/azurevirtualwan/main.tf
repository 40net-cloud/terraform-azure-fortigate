...
locals {
  ...
  nva_app_name = "${var.prefix***REMOVED***-vwan-fgt"
***REMOVED***
...
resource "azurerm_virtual_wan" "vwan" {
  name                = "${var.prefix***REMOVED***-virtualwan"
...
resource "azurerm_virtual_hub" "vhub" {
  name                = "${var.prefix***REMOVED***-virtualwan-hub"
  resource_group_name = azurerm_resource_group.resourcegroup.name
  location            = azurerm_resource_group.resourcegroup.location
...
resource "azurerm_public_ip" "elb-pip" {
  name                = "${var.prefix***REMOVED***-elb-pip"
  location            = var.location
  resource_group_name = azurerm_resource_group.resourcegroup.name
  allocation_method   = "Static"
  domain_name_label   = "${var.prefix***REMOVED***-elb-pip"
  sku                 = "Standard"
***REMOVED***
...
module "fgt_nva" {
  ...
  ***REMOVED***
  ...
  subscription_id             = "/subscriptions/${var.subscription_id***REMOVED***"
...
#   name                = "${var.prefix***REMOVED***-spoke1-vnet"
...
#   name                  = "${var.prefix***REMOVED***-spoke2-lnx"
#   location              = azurerm_resource_group.resourcegroup.location
#   resource_group_name   = azurerm_resource_group.resourcegroup.name
...
#   os_disk {
#     name                 = "${var.prefix***REMOVED***-spoke2-lnx-osdisk"
#     caching              = "ReadWrite"
#     storage_account_type = "StandardSSD_LRS"
#   ***REMOVED***

#   computer_name                   = "${var.prefix***REMOVED***-spoke2-lnx"
#   admin_username                  = var.username
#   admin_password                  = var.password
#   disable_password_authentication = false
#   custom_data                     = base64encode(templatefile("${path.module***REMOVED***/vm-customdata.tftpl", {***REMOVED***))
...
resource "azurerm_virtual_hub_routing_intent" "vwan_hub" {
  ...
  routing_policy {
    ...
    next_hop     = [for s in data.azapi_resource_list.listNVA.output.value : s if length(regexall(".*${local.nva_app_name***REMOVED***.*", s.properties.cloudInitConfiguration)) > 0][0].id
  ***REMOVED***
***REMOVED***
...
#  value = [for s in data.azapi_resource_list.listNVA.output.value : s if length(regexall(".*${local.nva_app_name***REMOVED***.*", s.properties.cloudInitConfiguration)) > 0][0].id
#***REMOVED***
