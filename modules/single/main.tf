...
locals {
  ...
  fgt_customdata = base64encode(templatefile("${path.module***REMOVED***/fgt-customdata.tftpl", var.fgt_customdata_variables))
***REMOVED***
...
resource "azurerm_managed_disk" "fgtvm-datadisk" {
  ...
  name                 = "${local.fgt_name***REMOVED***-datadisk-${count.index***REMOVED***"
  location             = var.location
  resource_group_name  = var.resource_group_name
  storage_account_type = "Standard_LRS"
  create_option        = "Empty"
